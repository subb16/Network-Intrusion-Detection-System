...
def correl():
    st.markdown("<h1 style='text-align: left; color: white; font-size: 20px;'>Correlation Heatmap</h1>", unsafe_allow_html=True)
    df = db_access(option)
...
def netactiv():
    df = db_access(option)
    fig = px.line(df, x='StartTime', y=['TotPkts', 'TotBytes'], title='Network Activity Over Time')
